...
public class RockGolem : Enemy
{
    ...
    public void Die()
    {
        Effect("Kill", transform.GetChild(1).gameObject, 0.7f);
        OnDamage();
    }
    void Netuarl()
    {

        Effect("Attack03", transform.GetChild(1).gameObject, 0.7f);
    ...
    void OnDamage()
    {
        Effect("Damage", this.transform.GetChild(0).gameObject, 0.4f);
        PlayerSound();
        player.Sound.Play();
    ...
    public IEnumerator Move()
    {
        ...
        while (MoveToNextNode(new Vector3(Objects.ObjectsList[0].position.x + 3, Objects.ObjectsList[0].position.y, Objects.ObjectsList[0].position.z)))
        {
