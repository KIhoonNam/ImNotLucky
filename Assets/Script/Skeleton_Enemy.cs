...
public class Skeleton_Enemy : Enemy
{
    ...
    private void Start()
    {
        ...
        sound = new string[4]{"SKELETON_HITTEN", "SKELETON_SKILL", "SKELETON_DEATH", "SKELETON_ATTACK" };
    ...
    public void Die()
    {
        Effect("Kill", transform.GetChild(1).gameObject,0,0.2f);
        OnDamage();
    }
    void Netuarl()
    {
        
            Effect("Attack03", transform.GetChild(1).gameObject,0,0.2f);
      
    }
    public void Effect(string name, GameObject transform,float Roate,float Scale)
    {
        ...
        if(Roate != 0)
        {
            Effects.transform.rotation = Quaternion.Euler(0, Roate, 0);
        }
    ...
    void OnDamage()
    {
        ...
        Effect("Damage", this.transform.GetChild(0).gameObject,180f,0.2f);
        PlayerSound();
        player.Sound.Play();
